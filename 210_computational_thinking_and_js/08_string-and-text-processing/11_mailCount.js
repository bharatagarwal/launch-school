function mailCount(emailData) {
  ...
  let dates = emails.map((email) => {
    return email.match(/Date:\s+([0-9\-]*)/g)[0].replace(/Date:\s+/, '');
  });

  let totalEmails = emails.length;
  let sortedDates = dates.sort();

  console.log(`Count of Email: ${totalEmails}`);

  let earliestDate = new Date(sortedDates[0]).toDateString();
  let latestDate = new Date(sortedDates[sortedDates.length - 1]).toDateString();
