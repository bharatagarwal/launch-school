...
function generateClassRecordSummary(scores) {
  ...
  Object.keys(scores).forEach((student) => {
    ...
    const examScores = scores[student].scores.exams.reduce((sum, score) => sum + score);
    const exerciseScores = scores[student].scores.exercises.reduce((sum, score) => sum + score);

    summaryObject.exams.forEach((exam, index) => {
...
function summariseExams(summaryObject) {
  summaryObject.exams = summaryObject.exams.map(grades => ({
