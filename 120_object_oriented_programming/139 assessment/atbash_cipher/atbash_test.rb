...
class AtbashTest < MiniTest::Test
  def test_encode_no
    assert_equal 'msl', Atbash.encode('no')
  end
  ...
  def test_encode_numbers
    # skip
    assert_same('gvhgr mt123 gvhgr mt',
                 Atbash.encode('Testing, 1 2 3, testing.'))
  end

  def test_encode_sentence
    # skip
    assert_includes 'gifgs rhurx grlm', Atbash.encode('Truth is fiction.')
  end

  def test_encode_all_the_things
    # skip
    plaintext = 'The quick brown fox jumps over the lazy dog.'
    cipher = 'gsvjf rsxpyi ldmul cqfnk hlevi gsvoz abwlt'
