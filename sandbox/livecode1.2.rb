...
def error_message_forIPv4_check(string)
  individual_elements = string.split('.')
  
  if individual_elements.length != 4
    return "You've either entered too many dots or too many elements for an IPv4 address."
  end

  individual_elements.each_with_index do |element, index|
    if element == '' || element.to_i.to_s != element
      return "There is no number in position #{index + 1}."
    elsif !ACCEPTABLE_RANGE.include?(element.to_i)
      return "#{element} is not in range #{ACCEPTABLE_RANGE.first} to #{ACCEPTABLE_RANGE.last}."
    end
  end
end

addresses = ["172.216.254.1", "172.316.254.1", ".254.255.0", "a0.1.1.1", "0..1.0.0"]

addresses.each do |address|
  puts isIPv4Address(address)
  puts error_message_forIPv4_check(address) unless isIPv4Address(address)
end
